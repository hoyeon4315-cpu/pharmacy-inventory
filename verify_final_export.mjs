...
function applyStatsReportDesign(ws, opts) {
  ...
  for (var R = 0; R <= tpnRow; R++) {
    for (var C = 0; C <= lastCol; C++) {
      ...
      else if (R === totalRow) { if (C < lastCol) cell.s.fill = totalFill; else cell.s.fill = headerFill; top = (C === lastCol) ? thin : medium; }
  ...
  if (footerRow != null) {
    ...
    fa.s.font = fontData; fb.s.font = fontData;
...
const lastDataRow = firstDataRow + drugCount - 1;
const totalRowNum = lastDataRow + 1;
const tpnRowNum = totalRowNum + 1;
const footerRowNum = tpnRowNum + 2;

const aoa = [['2026년 4월 항암제 조제통계']];
aoa.push([''].concat(depts).concat(['합계']));
for (let i = 0; i < drugCount; i++) aoa.push(['Drug' + i].concat(new Array(depts.length).fill('')).concat(['']));
aoa.push(['TOTAL'].concat(new Array(depts.length).fill(null)).concat([null]));
aoa.push(['특수TPN', 0].concat(new Array(depts.length - 1).fill('')).concat([null]));
aoa.push(new Array(depts.length + 2).fill(''));
aoa.push(['외래주사실 :', '100건 '].concat(new Array(depts.length).fill('')));
...
const out = path.join(dir, 'public', '_final_export.xlsx');
